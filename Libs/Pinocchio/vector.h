...
public:
    ...
    Vector(const Real &m1, const Real &m2)
    {
        m[0] = m1;
        m[1] = m2;
        checkDim<2>(VO());
    }
    Vector(const Real &m1, const Real &m2, const Real &m3)
    {
        m[0] = m1;
        m[1] = m2;
        m[2] = m3;
    ...
    template <class R>
    Vector(const Vector<R, Dim> &other) { VO::assign(other, *this); }
    ...
    template <class F>
    Vector<typename F::result_type, Dim> apply(const F &func) const
    {
        return VO::apply(func, *this);
    }

    template <class F>
    Vector<typename F::result_type, Dim> apply(const F &func, const Self &other) const
    {
...
private:
    ...
    friend class Vector;
    template <int WantedDim>
    void checkDim(const _VectorPrivate::VecOp<WantedDim> &) const {}
...
public:
    ...
    {
        if ((int)m.size() <= n)
            const_cast<Vector<Real, -1> *>(this)->m.resize(n + 1);
        return m[n];
    }

    // basic recursive functions
    template <class F>
    Vector<typename F::result_type, -1> apply(const F &func) const
    {
    ...
    template <class F>
    Vector<typename F::result_type, -1> apply(const F &func, const Self &other) const
    {
        ...
        {
            transform(m.begin(), m.end(), other.m.begin(), out.begin(), func);
            for (int i = m.size(); i < (int)other.m.size(); ++i)
                out[i] = func(Real(), other.m[i]);
        }
        else
        {
            transform(m.begin(), m.begin() + (other.m.end() - other.m.begin()), other.m.begin(), out.begin(), func);
            for (int i = other.m.size(); i < (int)m.size(); ++i)
                out[i] = func(m[i], Real());
        }
        return Vector<typename F::result_type, -1>(out);
    }
    ...
    typename Accum::result_type accumulate(const Op &op, const Accum &accum, const Self &other) const
    {
        ...
        if (m.size() == other.m.size())
            for (int i = 1; i < (int)m.size(); ++i)
                out = accum(out, op(m[i], other.m[i]));
        else if (m.size() < other.m.size())
        {
            for (int i = 1; i < (int)m.size(); ++i)
                out = accum(out, op(m[i], other.m[i]));
            for (int i = m.size(); i < (int)other.m.size(); ++i)
                out = accum(out, op(Real(), other.m[i]));
        }
        else
        {
            for (int i = 1; i < (int)other.m.size(); ++i)
                out = accum(out, op(m[i], other.m[i]));
            for (int i = other.m.size(); i < (int)m.size(); ++i)
                out = accum(out, op(m[i], Real()));
        }
        return out;
    }
...
#define VRD1 Vector<R1, D>
    ...
    public:
        ...
        static void assign(const VRD1 &from, VRD &to)
        {
            to[last] = from[last];
            Next::assign(from, to);
        }
        ...
        template <class R, class R1, int D>
        static void assign(const R1 &from, VRD &to)
        {
            to[last] = from;
            Next::assign(from, to);
        }
        ...
        {
            out[last] = func(v[last]);
            Next::_apply(func, v, out);
        }

        template <class R, int D, class F>
        static void _apply(const F &func, const VRD &v, const VRD &other, Vector<typename F::result_type, D> &out)
        {
            out[last] = func(v[last], other[last]);
            Next::_apply(func, v, other, out);
        }
    };
    ...
    private:
        ...
        template <class R, class R1, int D>
        static void assign(const VRD1 &from, VRD &to) { to[0] = from[0]; }

        template <class R, class R1, int D>
        static void assign(const R1 &from, VRD &to) { to[0] = from; }
...
public:
    ...
    template <class R, int D>
    static unsigned int less(const VRD &v1, const VRD &v2)
    {
        return ((v1[last] < v2[last]) ? (1 << last) : 0) + Next::less(v1, v2);
    }

    template <class R, int D>
    static void assignCorner(int idx, const VRD &v1, const VRD &v2, VRD &out)
    {
        out[last] = (idx & (1 << last)) ? v1[last] : v2[last];
        Next::assignCorner(idx, v1, v2, out);
    }
};

template <>
class BitComparator<1>
{
public:
    template <class R, int D>
    static unsigned int less(const VRD &v1, const VRD &v2)
